...
FILE_MAP_ROWS = [
    ...
    ["src/app/demo/custom-page.component.{ts,html}", "自研页面（/custom）= 左 demo + 右编辑器", "自研"],
    ["src/app/demo/demo.component.{ts,html}", "自研 demo，用 t()/label() 读文案", "自研"],
    ["i18n/projects/i18n/editor/editor.component.{ts,html}", "库：可选入口 i18n/editor（Monaco 编辑器）", "自研"],
    ["i18n/projects/i18n/src/lib/i18n.service.ts", "库：门面（语言切换 + t()/label()、applyEdited、下载/重置）", "自研"],
    ["i18n/projects/i18n/src/lib/format.service.ts", "库：Intl 格式化（日期/数字/货币/相对时间等）", "自研"],
    ["i18n/projects/i18n/src/lib/provide-i18n.ts + locale.service.ts", "库：provideI18n() 与语言状态服务", "自研"],
    ["src/app/i18n-bindings/{translations.json,i18n-keys.ts,source-messages.ts,app-i18n.ts}", "应用侧绑定：主文件 + 生成文件 + APP_I18N", "自研"],
    ...
    ["package.json", "脚本入口：i18n:check / i18n:split / build:compile、lib:pack（构建库并固定名打包安装）等", "全部"],
]
