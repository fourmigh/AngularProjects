...
const OUT_DIR = join(MODULE, 'assets', 'locale');
const KEYS_FILE = join(MODULE, 'i18n-keys.ts');
const SOURCE_MESSAGES_FILE = join(MODULE, 'source-messages.ts');
...
const localeUnion = LANGUAGES.map((l) => `'${l}'`).join(' | ');
